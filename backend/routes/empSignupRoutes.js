...
employeerouter.post("/register/employer", async (req, res) => {
    ...
    const query = `INSERT INTO employers (company_name, email, password_hash, phone, location, website, industry, company_size) 
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;

    db.query(query, [companyName, companyEmail, hashedPassword, phone, address, website, industry, companySize], (err, result) => {
